...
class UmengPusher
{
    ...
    public function __construct($settings = [])
    {
        if (! empty($settings["ios"])) {
            ...
            $this->ios = new IOSPusher($iosAppKey, $iosAppMasterSecret,$settings["product"]);
        }
        if (! empty($settings["android"])) {
            $androidAppKey = $settings["android"]['app_key'];
            $androidAppMasterSecret = $settings["android"]['app_master_secret'];
            $this->android = new AndroidPusher($androidAppKey, $androidAppMasterSecret,$settings["product"]);
        }
    }
